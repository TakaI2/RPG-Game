...
- **仮説**: StoryScene.endStory()で手動cleanup()を呼んでから scene.start()しているため、シーンが不正な状態になっている可能性
- **次のアクション**: cleanup処理をshutdownイベントに委譲する

### 問題2: ポーズメニューのボタンが押せない
- **症状**: ポーズ画面でボタンを押そうとしても押せない（位置ずれの可能性）
- **確認済み**:
  - 深度設定は正しい（PauseMenu: 10000 > VirtualJoystick/AttackButton: 9000）
  - PauseMenuのcontainerには`setScrollFactor(0)`が設定されている
- **仮説**: createButton()で作成された子containerには`scrollFactor`が設定されていない可能性
- **次のアクション**: ボタンcontainerにも`setScrollFactor(0)`を明示的に設定する

## ファイル変更履歴

### 修正済みファイル (2025-11-16)
- `src/scenes/StoryScene.ts` - シーン遷移ロジックの追加（game → MainSceneのマッピング）、BGM停止処理追加
...
- `src/ui/PauseMenu.ts` - scrollFactor(0)追加、depth調整
